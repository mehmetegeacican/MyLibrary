import { Switch, Box, RadioGroup } from "@mui/material";
...
const StyledSwitch = styled(Switch)(({ theme }) => ({
    ...
    '& .MuiSwitch-thumb': {
        backgroundColor: theme.palette.mode === 'dark' ? '#4a90d9' : '#f5a623',
        width: 32,
        height: 32,
        '&:before': {
            content: "''",
...
export default function LightAndDarkSwitch() {
    const {theme, dispatch} = useAuthContext()
    const isDark = theme === LIGHT_THEMES.DARK;
    const handleColorSwitch = () => {
        const updatedTheme = theme === LIGHT_THEMES.DARK ? LIGHT_THEMES.LIGHT : LIGHT_THEMES.DARK;
        dispatch({
            type:'SWTICH_THEME',
    ...
    return (
        <RadioGroup name="use-radio-group" value={theme} onChange={handleColorSwitch}>
            <Box sx={{
                display: 'flex',
                flexDirection: 'row',
                alignItems: 'center',
                ...
                <LightModeIcon sx={{ color: isDark ? 'text.disabled' : '#f5a623' }} />
                <StyledSwitch
                    checked={isDark}
                    onChange={handleColorSwitch}
                />
                <DarkModeIcon sx={{ color: isDark ? '#4a90d9' : 'text.disabled' }} />
            </Box>
        </RadioGroup>
    )
}
