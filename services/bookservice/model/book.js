 ...
 */
const executeInsertNewBook = async (bookName, description, bookCategories, bookStatus, bookAuthors,userId,imagePath="") => {
    //Step 1 -- Open the Db
    let client = await connectDb();
    ...
    try {
        ...
        const insertQuery = `INSERT INTO books (name, description, entered, category, status,authors,"imagePath",user_id) VALUES($1, $2, $3, $4, $5,$6,$7,$8)`;
        const values = [bookName, description, date, category, bookStatus,authors,imagePath,userId];
 ...
 * @returns 
 */
const executeUpdateBook = async (id,bookName,author,bookCategories,bookStatus,bookAuthors, imagePath = "") => {
    //Step 1 -- Open the Db
    let client = await connectDb();
    try{
        const updateQuery = `UPDATE books SET "name"=$1, description=$2, category=$3, status=$4, authors = $5 , "imagePath"=$6 WHERE id=$7`;
        const values = [bookName,author,formatDatas(bookCategories),bookStatus,formatDatas(bookAuthors),imagePath,id];
